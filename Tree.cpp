...
template<class T>
bool PathToAnyNode(Node<T>* root, int item, vector<T> v)
{
	if (root == NULL)
		return false;
	...
	bool found = PathToAnyNode(root->left, item, v);
	
	if (!found)
	{
		found = PathToAnyNode(root->right, item, v);
	}
